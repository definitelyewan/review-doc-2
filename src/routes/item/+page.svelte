...
<div class="md:mx-32 mx-2 my-2">
	<div class="flex flex-wrap gap-4 justify-center">
		{#each slicedSource(filteredBlocks) as item}
			<div class="card rounded-lg outline-2 outline-gray-300 drop-shadow-lg relative w-full sm:w-1/3" style="background-image: url({item.banner}); background-size: cover; background-position: center;">
				...
				<div class="relative z-10 flex flex-row items-center gap-2">
					<ItemCover cover={item.url} id={item.id} scroll={false}/>
					<div>
						<p class="md:text-2xl text-xl drop-shadow-lg text-shadow-outline dark:text-white text-[#121212]">{item.name}</p>
						<p class="text-md drop-shadow-lg text-shadow-outline dark:text-white text-[#121212]">Released: {item.date}</p>
					</div>
				</div>
			</div>
		{/each}
